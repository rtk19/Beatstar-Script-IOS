...
export default class SongTemplate {
  ...
  test() {
    const songId = (this.id >>> 0).toString(16).padStart(8, "0");
    const artworkAssetId = `${songId}${songId}${songId}00000001`;
    const audioAssetId = `${songId}${songId}${songId}00000002`;
    this.BPM = 140;
    ...
    this.LegalAttribution = "";
    this.SilentBeats = 0;
    // Beatstar uses both of these fields as playback/cache identities. Leaving
    // either at its default makes consecutive custom songs look identical.
    this.MusicFileSourceID = (this.id >>> 0) || 1;
